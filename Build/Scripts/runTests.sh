...
read -r -d '' HELP <<EOF
cookieman test runner. Execute unit test suite and some other details.
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="public/typo3conf/ext/cookieman/${1}"
else
    case ${TEST_SUITE} in
        unit)
            TEST_FILE="public/typo3conf/ext/cookieman/Tests/Unit"
            ;;
    esac
fi
